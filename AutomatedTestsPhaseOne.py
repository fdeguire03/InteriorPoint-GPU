...
def test_phase_one_gradient(use_cupy = False, tol = 1e-8):
    ...
    phaseOne = phase_one(G, h, mu, use_cupy = use_cupy)
    ...
    true_grad_x = np.array([-1, -3]) + np.array([-1, 1]) / 5 + \
                  np.array([1, -2]) / 2 + np.array([1, 4]) / 10
...
def test_phase_one_hessian(use_cupy = False, tol = 1e-8):
    ...
    true_hess_xx = np.array([[1, 2, 3], 
                ...
                np.array([[16, 20, 24], 
                          [20, 25, 30], 
                          [24, 30, 36]])
    true_hess_xs = np.reshape(- np.array([1, 2, 3]) / 81 - np.array([4, 5, 6]),
                              newshape = (3, 1))
...
def test_initialized_phase_one(use_cupy = False, linear_solver = "solve"):
  ...
  """
  if use_cupy:
    print("Testing phase one given an intialization with cupy and solver '" + \
          linear_solver + "'")
  else:
    print("Testing phase one given an initialization without cupy with solver '" +\
          linear_solver + "'")
  # Set which phase_one is not initialized in
  G = np.array([[-1, -3], 
                [-1, 1],
  ...
  phaseOne = phase_one(G, h, mu, x0 = x0, use_cupy = use_cupy,
                       linear_solver = linear_solver)
  x, s, _ = phaseOne.solve()
  if use_cupy:
      x = cp.asnumpy(x)
...
def automated_tests():
    ...
    tests = [test_phase_one_objective, test_phase_one_gradient, 
            test_phase_one_hessian, test_initialized_phase_one, test_phase_one]
